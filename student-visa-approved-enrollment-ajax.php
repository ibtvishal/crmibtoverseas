...
if($_SESSION['level_id']==15 || $_SESSION['level_id'] == 19 || $_SESSION['level_id']==25){
    $branch_id = getField('branch_id',$tbl_admin,$_SESSION['sess_admin_id']);
    $b_id =  " and a.branch_id in ($branch_id)";
}
if($_SESSION['level_id']==16){
...
INNER JOIN $tbl_student_status AS b on b.stu_id = a.id $join  LEFT JOIN $tbl_student_enrollment as c on c.stu_id = a.id
...
$nestedData=array(); 
    ...
    // if($line->visa_issue_date != ''){
    //   $visa_issue_date = $line->visa_issue_date;
    // }else{
    //   $visa_issue_date = $line->visa_approved_cdate;
    // }
    ...
    $rSqla = $obj->query("select * from $tbl_student_enrollment_remark where stu_id='".$line->student_id."' and type=0 order by id desc");
      ...
      if($obj->numRows($rSqla) > 0){
        ...
        if($line->fee_payment == 'Pending (Cheque Received)'){
          $cheque = '<a href="uploads/'.$line->cheque_pic.'" target="_blank"><i class="fa fa-eye"></i></a>';
        }
    ...
    $nestedData[] = "<span ".$color.">".$line->passport_status."</span>";
    $nestedData[] = "<span ".$color.">".$line->after_visa_fee."</span>";
    // $nestedData[] = "<span ".$color.">".$line->enrollment_status."</span>";
    $nestedData[] = "<span ".$color.">".$line->mail_id."</span>";
    $nestedData[] = "<span ".$color.">".$line->mail_password."</span>";
    $nestedData[] = "<span ".$color.">".$line->university_portal_login."</span>";
    $nestedData[] = "<span ".$color.">".$line->university_portal_password."</span>";
    $nestedData[] = "<span ".$color.">".$line->advisor_meeting."</span>";
    $nestedData[] = "<span ".$color.">".$line->classes_start_date."</span>";
    $nestedData[] = "<span ".$color.">".$line->total_fee."</span>";
    $nestedData[] = "<span ".$color.">".$line->minimum_fee."</span>";
    $nestedData[] = "<span ".$color.">".$line->rate."</span>";
    $nestedData[] = "<span ".$color.">".$line->fee_in_inr."</span>";
    $nestedData[] = "<span ".$color.">".$line->fee_payment.$cheque."</span>";
    $nestedData[] = "<span ".$color.">".$line->sattlement_amount."</span>";
    $nestedData[] = "<span ".$color.">".$line->sattlement_amount_status."</span>";
    $nestedData[] = "<span ".$color.">".$line->fee_paid_through."</span>";
    $nestedData[] = "<span ".$color.">".$line->payment_method."</span>";
    $nestedData[] = "<span ".$color.">".$line->tt_receipt_no."</span>";
    ...
    // $nestedData[] = "<span ".$color.">".$line->video_shoot."</span>";
