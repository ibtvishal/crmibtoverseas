...
if($_SESSION['level_id']!=1){
    ...
    $whr1 .= " and b.branch_id in ($branch_id)";
}
...
if($_REQUEST['payment']){
  ...
  if($_REQUEST['filter_start_date'] && !$_REQUEST['filter_end_date']){
    ...
    $whr .= " and date(a.cdate) = '$filter_start_date'";
    $whr1 .= " and date(a.cdate) = '$filter_start_date'";
  }
  if(!$_REQUEST['filter_start_date'] && $_REQUEST['filter_end_date']){
    $filter_end_date = $_REQUEST['filter_end_date'];
    $whr .= " and date(a.cdate) = '$filter_end_date'";
    $whr1 .= " and date(a.cdate) = '$filter_end_date'";
  }
  if($_REQUEST['filter_start_date'] && $_REQUEST['filter_end_date']){
    $filter_start_date = $_REQUEST['filter_start_date'];
    $filter_end_date = $_REQUEST['filter_end_date'];
...
<body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <select name="payment" id="payment" class="form-control" onchange="submit_form()">
                                ...
                                <option value="Pending" <?=$_REQUEST['payment'] == 'Pending' ? 'selected' : '' ?>>Pending
                                </option>
                                <option value="1" <?=$_REQUEST['payment'] == '1' ? 'selected' : '' ?>>Approved
                                </option>
                                <option value="1" <?=$_REQUEST['payment'] == '2' ? 'selected' : '' ?>>Disapproved
                                </option>
                            </select>
                        </div>
                        <div class="col-md-3">
    ...
    var dataTable = $('#datable_3s').DataTable({
        ...
        "ajax": {
            url: "pending-enrollment-ajax.php",
            type: "post",
            error: function() {
