...
if($_SESSION['join']!=''){
  ...
  $sql="SELECT a.*,b.cstatus,c.enrollment_intake, CASE 
       ...
       END AS visa_approved_date FROM $tbl_student AS a
          ...
          WHERE b.cstatus = 'Visa Refused' $b_id  $whr1 $half";
...
$nestedData=array(); 
  ...
  if($showval == 1){
    ...
    $get_refused = $obj->fetchNextObject($obj->query("select tution_fee from $tbl_student_refused_refund where stu_id='".$line->id."'"));
    $get_refused_ihs = $obj->fetchNextObject($obj->query("select ihs_amount from $tbl_student_refused_ihs_refund where stu_id='".$line->id."'"));
    ...
    $nestedData[] = "<span ".$color.">".$get_refused->tution_fee."</span>";
    $nestedData[] = "<span ".$color.">".$get_refused_ihs->ihs_amount."</span>";
    ...
    $nestedData[] = "<a href='student-editf.php?id=".base64_encode(base64_encode(base64_encode($line->id)))."&type=refused'><i class='fa fa-edit' style='margin-right: 6px;font-size: 16px;'></i> </a>";
