            ...
            <div class="container-fluid">
                <div class="row heading-bg">
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                        <h5 class="txt-dark">CRM Incentive</h5>
                    </div>
                                            ...
                                            <table id="datable_2" class="table table-hover display pb-30">
                                                ...
                                                <tbody>
                                                    <?php
                                                            ...
                                                            $sql = $obj->query("SELECT id, name FROM $tbl_admin WHERE level_id = 9 $whr ORDER BY id DESC", $debug = -1);
                                                            while ($line = $obj->fetchNextObject($sql)) {
                                                                $res = $obj->fetchNextObject($obj->query("SELECT SUM(amount) AS total, month, year FROM $insentive_calculated WHERE user_id = '{$line->id}' AND month = '$month' AND year = '$year' AND type = 'CRM Visit' GROUP BY year, month", $debug = -1));
                                                                $total_lead = $obj->numRows($obj->query("SELECT * from $tbl_lead where crm_executive_id='{$line->id}' AND month(cdate) = '$month' AND year(cdate) = '$year'", $debug = -1));
                                                                $total_visit = $obj->numRows($obj->query("SELECT * from $tbl_lead  as a join $tbl_visit as b on  a.applicant_contact_no=b.applicant_contact_no or a.applicant_alternate_no=b.applicant_contact_no or a.applicant_alternate_no=b.applicant_alternate_no or a.applicant_contact_no = b.applicant_alternate_no where a.crm_executive_id='{$line->id}' AND month(b.cdate) = '$month' AND year(b.cdate) = '$year'", $debug = -1));
                                                                ?>
                                                    <tr>
                                                        <td><?= $line->id ?></td>
                                                        <td><?= $line->name ?></td>
                        ...
                        <div class="panel panel-default card-view">
                            <div class="panel-wrapper collapse in">
                                <div class="panel-body">
                                    <div class="table-wrap">
                                        <div class="table-responsive">
                                            <h5>CRM Enrollments Incentive</h5>
                                            <table id="datable_3" class="table table-hover display pb-30">
                                                ...
                                                <tbody>
                                                    <?php
                                                            ...
                                                            $sql = $obj->query("SELECT id, name FROM $tbl_admin WHERE level_id = 9 $whr ORDER BY id DESC", $debug = -1);
                                                            while ($line = $obj->fetchNextObject($sql)) {
                                                                $res = $obj->fetchNextObject($obj->query("SELECT SUM(amount) AS total, month, year FROM $insentive_calculated WHERE user_id = '{$line->id}' AND month = '$month' AND year = '$year' AND type = 'CRM' GROUP BY year, month", $debug = -1));

                                                                $total_lead = $obj->numRows($obj->query("SELECT * from $tbl_lead where crm_executive_id='{$line->id}' AND month(cdate) = '$month' AND year(cdate) = '$year'", $debug = -1));
                                                                $total_enrollment = $obj->numRows($obj->query("SELECT * from $tbl_lead  as a join $tbl_student as b on  a.applicant_contact_no=b.student_contact_no or a.applicant_alternate_no=b.student_contact_no or a.applicant_alternate_no=b.alternate_contact or a.applicant_contact_no = b.alternate_contact where a.crm_executive_id='{$line->id}' AND month(b.cdate) = '$month' AND year(b.cdate) = '$year'", $debug = -1));
                                                                ?>
                                                    <tr>
                                                        <td><?= $line->id ?></td>
                                                        <td><?= $line->name ?></td>
