...
$sql=$obj->query("select COUNT(*) as num_rows from $tbl_visit  where 1=1 $whr",$debug=-1);
...
$totalFiltered = $totalData; 
  ...
  $sql="select * from $tbl_visit   where 1=1 $whr";
...
while($line=$obj->fetchNextObject($query)) { 
      ...
      $slip_data11 = "<a style='padding: 5px 10px;' class='btn btn-success' onclick='get_modal(\"".base64_encode(base64_encode(base64_encode($line->id)))."\")'>Pay Now</a>";
        ...
        $slip_data11 = $result_fee_11->payment_type.' Paid';
      }
    ...
    $tsql = $obj->query("select id from $tbl_student where student_contact_no='".$line->applicant_contact_no."' or alternate_contact='".$line->applicant_contact_no."' or student_contact_no='".$line->applicant_alternate_no."' or alternate_contact='".$line->applicant_alternate_no."'");
      ...
      $nestedData[] = "<span ".$color.">".getField('name',$tbl_admin,$line->management_member)."</span>";
      }
      $nestedData[] = "<a target='_blank' href='visit-editf.php?id=".base64_encode(base64_encode(base64_encode($line->id)))."&meet'><i class='fa fa-edit' style='margin-right: 6px;font-size: 16px;'></i> </a>";
