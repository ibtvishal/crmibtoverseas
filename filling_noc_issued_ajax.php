...
if($pending != ''){
  $sql=$obj->query("select a.id as num_rows from $tbl_student as a INNER JOIN $tbl_student_filing_noc f ON a.id = f.stu_id  where 1=1 and f.value = 'Affidavit' and f.status=2 AND (
    SELECT COUNT(*) 
...
) > 0   $whr $whr2 group by a.id",$debug=-1);
  // $sql=$obj->query("select a.id as num_rows from $tbl_student as a inner join $tbl_student_document as b on a.id = b.stu_id INNER JOIN $tbl_student_filing_noc f ON a.id = f.stu_id  where 1=1 and b.dtype in (24,39) and f.value = 'With_Affidavit' AND NOT EXISTS ( SELECT 1 FROM $tbl_student_filing_noc f2 WHERE f2.stu_id = a.id AND f2.value = 'Financials' ) $whr $whr1 group by a.id",$debug=-1);
  $line=$obj->numRows($sql);
  $totalData=$line;
  $totalFiltered = $totalData; 
  
  
  $sql="select a.*,f.user_id as user_id_f,f.cdate as cdate_f from $tbl_student as a INNER JOIN $tbl_student_filing_noc f ON a.id = f.stu_id  where 1=1 and f.value = 'Affidavit' and f.status=2 AND (
    SELECT COUNT(*) 
    FROM $tbl_student_filing_noc f1 
    WHERE f1.stu_id = a.id AND f1.status = 1
) = 0 
AND (
  ...
  // $sql="select a.*,b.verify,b.user_id,b.with_financial_verify  from $tbl_student as a inner join $tbl_student_document as b on a.id = b.stu_id INNER JOIN $tbl_student_filing_noc f ON a.id = f.stu_id  where 1=1 and b.dtype in (24,39) and f.value = 'With_Affidavit' AND NOT EXISTS ( SELECT 1 FROM $tbl_student_filing_noc f2 WHERE f2.stu_id = a.id AND f2.value = 'Financials' ) $whr $whr1";
}else{
  if($_SESSION['whr']!=''){
    $whr = $_SESSION['whr'];
  }else{
    if($second != ''){
      $whr = ' and b.dtype in (39)';
    }else{
      $whr = ' and b.dtype in (24)';
    }
  }
  ...
  $totalData=$line;
  $totalFiltered = $totalData; 
  $sql="select a.*,b.verify,b.user_id,b.with_financial_verify  from $tbl_student as a inner join $tbl_student_document as b on a.id = b.stu_id  where 1=1  $whr $whr1";
}
...
while ($line = $obj->fetchNextObject($query)) {
    $nestedData = array();
    $doc = [];
    $fsql23 = $obj->query("select * from $tbl_student_document where stu_id='{$line->id}' and dtype=39 and status=1 order by id desc");
    while($fResult23 = $obj->fetchNextObject($fsql23)){
        $doc[] = '<a href="javascript:void(0);"class="documnet-view" onclick="documentview(1,'.$fResult23->id.')"><i class="fa fa-eye"></i></a>';
    }
    $docs = implode(', ',$doc);

    $doc1 = [];
    $fsql23 = $obj->query("select * from $tbl_student_document where stu_id='{$line->id}' and dtype=24 and status=1 order by id desc");
    while($fResult23 = $obj->fetchNextObject($fsql23)){
    ...
    $btn1 = '';
    if(count($doc) > 0){
    if($line->with_financial_verify == 1){
      $btn = '<a id="change_hide_status'.$line->id.'" class="btn btn-success">Verfied</a>' ;
    }elseif($line->with_financial_verify == 2){
      $btn = '<a id="change_hide_status'.$line->id.'" class="btn btn-danger" onclick="change_hide_status('.$line->id.')">Disapproved</a>' ;
    }else{
      $btn = '<a id="change_hide_status'.$line->id.'" onclick="change_hide_status('.$line->id.')" class="btn btn-primary">Verification Pending</a>';
    }
  }
    ...
    while ($urResult = $obj->fetchNextObject($urSql)) {
        if (!empty($urResult->offical_email)) { 
    ...
    $textarea1 = '';
    if(count($doc) > 0){
    $textarea = "<textarea onchange='change_remakrs(this.value,$line->id)'>$line->affidavit_remarks</textarea><span class='text-success' id='success$line->id'></span>";
    }
    if(count($doc1) > 0){
