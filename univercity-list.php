				...
				<div class="container-fluid">
					...
					<div class="row heading-bg">
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
						  <h5 class="txt-dark">Manage University</h5>
						</div>
						...
						<div class="breadcrumb-section col-lg-4 col-sm-8 col-md-8 col-xs-12">
						  <ol class="breadcrumb">
							...
							<li class="active"><span><a href="javascript:void();" onclick="ShowModal();">Add University</a></span></li>
											...
											<table id="datable_1" class="table table-hover display  pb-30" >
												...
												<tbody>
													...
													<tr>
														...
														<td><?php echo $line->video_url ?></td>
														<td><?php echo $line->video_title ?></td>
														<td><?php echo $line->fee ?></td>
														<td>  <div class="material-switch">
															<input id="someSwitchOptionPrimary<?php echo $i; ?>"  type="checkbox" class="chkstatus" value="<?php echo $line->id;?>" <?php echo ($line->status=="1")?'checked':'' ?> data-one="<?php echo $tbl_univercity?>"/>
							...
							<div class="modal-content">
								...
								<div class="modal-header">
									...
									<span id="err_video_title" style="color:red;"></span>
										<div class="form-group">
											<label for="recipient-name" class="control-label mb-10"> Fee:</label>
											<input type="text" class="form-control" id="fee" name="fee">
										</div>
									
									<span id="err_fee" style="color:red;"></span>
									</div>
...
<script>
	$(document).ready(function(){
			...
			$('#country_id').change(function() {
				...
				data: {country_id:country_id,type:'getstateforuniversity'}, //set data
...
{	
	...
	$.ajax({
        type: "GET", 
        url: 'ajax/getModalData.php',
        data: {id:id,type:'getUnivercity'}, //set data
        beforeSend: function () {
        },
        success: function (response) {
            response = response.split('##');
            ...
            $("#name").val(response[2]);
            $("#video_url").val(response[4]);
            $("#video_title").val(response[5]);
            $("#fee").val(response[6]);

            getState(response[1],response[3]);
            // $("#state_id").val(response[3]);
          	$("#exampleModal").modal('show');
        }
    });
...
$("#btnSubmit").on("click", function() {	
    ...
    if(fee==''){
		$("#err_fee").show().html("This field is required.");
		return;
	}
    if(id==''){
    	action='addUnivercity';
    }else{
    	action='updateUnivercity';    	
    }
	$.ajax({
        type: "POST", 
        url: 'ajax/submitData.php', 
        data: {'id':id,'name':name,'country_id':country_id,'state_id':state_id,'video_url':video_url,'video_title':video_title,'fee':fee,'action':action}, 
        success: function (response) {
