...
$sql=$obj->query("select COUNT(a.id) as num_rows from $tbl_visit as a inner join $tbl_visit_fee as b on a.id = b.visit_id  where  1=1 and b.payment_type='Registration'  $whr1",$debug=-1);
...
$sql="select a.*, b.visa_type as visa_type_p, b.payment_date  from $tbl_visit as a inner join $tbl_visit_fee as b on a.id = b.visit_id  where  1=1 and b.payment_type='Registration'  $whr1";
// echo $sql; die;
$query = $obj->query($sql);
...
while($line=$obj->fetchNextObject($query)) { 
    ...
    if($result_fee_count1 == 0){
      $color = 'style="color:red"';
       $poay_now = "<a  style='padding: 5px 10px;' class='btn btn-success'  href='add-fee.php?id=".base64_encode(base64_encode(base64_encode($line->id)))."&type=Enrollment'>Pay Now</a>";
    }else{
      $poay_now = 'Paid';
      $color = '';
    }
    ...
    $student = $obj->fetchNextObject($get_student);
      ...
      $nestedData[] = "<span ".$color.">".$student->student_contact_no."</span>";
      ...
      $nestedData[] = "<a href='slip.php?id=".base64_encode(base64_encode(base64_encode($line->id)))."&type=Registration' target='_blank'><i class='fa-solid fa-file' style='margin-right: 6px;font-size: 16px;'></i></a>";
      ...
      $nestedData[] = "<a  style='padding: 5px 10px;' href='add-fee.php?id=".base64_encode(base64_encode(base64_encode($line->id)))."&type=Registration'><i class='fa fa-edit' style='margin-right: 6px;font-size: 16px;'></i> </a>";
      }
    $data[] = $nestedData;
    $i++;
  }
