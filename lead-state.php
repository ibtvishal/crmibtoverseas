				...
				<div class="container-fluid">
					...
					<div class="row heading-bg">
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
						  <h5 class="txt-dark">Manage Lead State</h5>
						</div>
						...
						<div class="breadcrumb-section col-lg-4 col-sm-8 col-md-8 col-xs-12">
						  <ol class="breadcrumb">
							...
							<li class="active"><span><a href="javascript:void();" onclick="ShowModal();">Add Lead State</a></span></li>
										...
										<div class="table-responsive">
											<table id="datable_1" class="table table-hover display  pb-30" >
												...
												<tbody>
													<?php
													$i=1;
													$sql=$obj->query("select * from $tbl_location_states where 1=1",$debug=-1);
													while($line=$obj->fetchNextObject($sql)){?>
													<tr>
														<td><?php echo $line->id ?></td>
														<td><?php echo getCountry($line->country_id) ?></td>
														<td><?php echo $line->name ?></td>
														<td>  <div class="material-switch">
															<input id="someSwitchOptionPrimary<?php echo $i; ?>"  type="checkbox" class="chkstatus" value="<?php echo $line->id;?>" <?php echo ($line->status=="1")?'checked':'' ?> data-one="<?php echo $tbl_location_states?>"/>
...
{	
	...
	$.ajax({
        ...
        data: {id:id,type:'getlocationstate'}, //set data
        beforeSend: function () {
        },
        success: function (response) {
            response = response.split('##');
            $("#id").val(response[0]);
            $("#state_name").val(response[1]);
            $("#country_id").val(response[2]);
          	$("#exampleModal").modal('show');
        }
    });
