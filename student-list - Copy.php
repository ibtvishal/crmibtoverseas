          ...
          <div class="col-sm-12">
            <div class="panel panel-default card-view">
              <div class="panel-wrapper">                              
                  <div class="col-md-3">
                    <div class="form-group">
                      <select name="transfer_branch_id[]" id="transfer_branch_id" class="form-control select2" multiple="" placeholder="Select Branch">                        
                        <option value="">Select Branch</option>
                        <?php
                        if(!empty($_REQUEST['transfer_branch_id'])){
                          $branchFilterArr = $_REQUEST['transfer_branch_id'];
                        }else{
                          $branchFilterArr = array();
                        }                            
                        $branchSql1 = $obj->query("select * from $tbl_branch where status=1");
                        while($branchResult1 = $obj->fetchNextObject($branchSql1)){?>
                          <option value="<?php echo $branchResult1->id; ?>" <?php if(sizeof($branchFilterArr)>0){ if(in_array($branchResult1->id,$branchFilterArr)){?> selected <?php }} ?>><?php echo $branchResult1->name; ?></option>
                        <?php }?>
                      </select>
                    </div>
                  </div>
                    ...
                    <div class="form-group">
                      <select name="transfer_user_from_id" id="transfer_user_from_id" class="form-control">
                        ...
                        if(!empty($_REQUEST['transfer_branch_id']) && !empty($_REQUEST['transfer_user_type'])){
                          $tidArr = $_REQUEST['transfer_branch_id'];
                          $t_user_type = $_REQUEST['transfer_user_type'];
                          $i=1;$twhr="";
                          foreach($tidArr as $val){
                            if($i==1){
                              $twhr .=" and ( FIND_IN_SET($val, branch_id)";
                            }else{
                              $twhr .=" or FIND_IN_SET($val, branch_id)";
                            }
                            if(count($tidArr)==$i){
                              $twhr .=")";
                            }
                          ...
                          $ssql = $obj->query("select * from $tbl_admin where status=1 and level_id='$t_user_type' $twhr",-1); //die();
                          while($sResult = $obj->fetchNextObject($ssql)){?>
                          ...
                          <tbody >
                            ...
                            if ($_SESSION['level_id']==4){
                              ...
                              if(!empty($_REQUEST['filling_manager_id'])){
                                $sql=$obj->query("select a.* from $tbl_student as a inner join $tbl_student_status as b ON  a.id=b.stu_id where 1=1 and a.country_id in (1,2,3,6) and b.stage_id in (3,30,8,24) and b.cstatus in ('Tuition Fees Paid','COE Received','I-20 Issued','CAS Received')",-1); //die;
                              }
                              ...
                              if(base64_decode(base64_decode(base64_decode($_GET['transfer'])))==1){
                                  if(!empty($_REQUEST['transfer_branch_id'])){
                                    ...
                                    foreach($tidArr as $val){
                                      if($i==1){
                                        $tswhr .=" and ( FIND_IN_SET($val, branch_id)";
                                      }else{
                                        $tswhr .=" or FIND_IN_SET($val, branch_id)";
                                      }
                                      if(count($tidArr)==$i){
                                        $tswhr .=")";
                                      }
