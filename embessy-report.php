...
if($_REQUEST['filter_start_date'] && $_REQUEST['filter_end_date']){
  ...
  $whr .= " and date(b.interview_date) >= '$filter_start_date' and date(b.interview_date) <= '$filter_end_date'";
}
...
<body>
    ...
    <div class="wrapper theme-1-active pimary-color-green">
            ...
            <div class="container-fluid">
                ...
                <div class="row heading-bg">
                    <div class="col-lg-6 col-md-4 col-sm-4 col-xs-12">
                        <h5 class="txt-dark">Manage Embessy Report
                ...
                <form method="post" name="searchfrm" id="searchfrm" action="embessy-report.php">
                    <div class="row">
                        <div class="col-sm-12">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div id="total_percentage" style="height:300px;margin-bottom: 20px;"></div>
                    </div>
                    <div class="col-md-4">
        ...
        <script>
        <?php
                    $get = $obj->query("SELECT b.interview_location,COUNT(*) as total FROM `$tbl_student` as a inner join $tbl_appointment as b on a.id=b.student_id inner join $tbl_student_status as c on a.id=c.stu_id where b.interview_location is not null and a.country_id='3' and c.cstatus = 'Visa Approved' $whr group by b.interview_location order by COUNT(*) desc",-1);
                    $totalapproved = 0;
                    $data = [];
                    while($res = $obj->fetchNextObject($get)){
                        $data[$res->interview_location] = ['name' => addslashes($res->interview_location), 'y' => $res->total];
        ...
        var chart = new CanvasJS.Chart("total_visa_approved", {
            ...
            title: {
                text: "USA Visa Approval Wise Percentage (<?=$totalapproved?>)",
                fontFamily: "Arial",
                fontWeight: "bold",
                fontSize: 14
            },
        ...
        <script>
        <?php
                    $get = $obj->query("SELECT b.interview_location,COUNT(*) as total FROM `$tbl_student` as a inner join $tbl_appointment as b on a.id=b.student_id  where b.interview_location is not null and a.country_id='3' $whr group by b.interview_location order by COUNT(*) desc",-1);
                    $totalapprovedz = 0;
                    $datas = [];
                    while($res = $obj->fetchNextObject($get)){
                        $datas[$res->interview_location] = ['name' => addslashes($res->interview_location), 'y' => $res->total];
                        $totalapproved += $res->total;
                    }
                ?>
        var chart = new CanvasJS.Chart("total_interview", {
            ...
            animationEnabled: true,
            title: {
                text: "USA Total Inteview Wise Percentage (<?=$totalapproved?>)",
                fontFamily: "Arial",
                fontWeight: "bold",
                fontSize: 14
            },
   ...
   <script>
var chart = new CanvasJS.Chart("total_percentage", {
    exportEnabled: true,
    animationEnabled: true,
    title: {
        text: "USA Visa Approval vs Total Interview Percentage",
        fontFamily: "Arial",
        fontWeight: "bold",
        fontSize: 14
    },
    data: [{
        type: "column",
        toolTipContent: "{label}:({y}%) - {total_approved} out of {total_interview}",
        indexLabel: "{y}%",
        fontFamily: "Arial",
        dataPoints: [
            <?php
            ...
            foreach ($datas as $key => $d) {
                $total_interview = $d['y'];
                $total_approved = $data[$key]['y'] ?? 0;
                $percentage = round(($total_approved / $total_interview) * 100, 2);
                $dataPoints[] = [
                    'label' => $d['name'],
                    'y' => $percentage,
            ...
            foreach ($dataPoints as $point) {
                echo "{ label: '" . $point['label'] . "', y: " . $point['y'] . " , total_interview: " . $point['total_interview'] . ", total_approved: " . $point['total_approved'] . "},";
            }
            ?>
        ]
    }]
