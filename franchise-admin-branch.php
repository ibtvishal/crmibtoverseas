...
<body>
    ...
    <div class="wrapper theme-1-active pimary-color-green">
        ...
        <?php
        $branch_id = base64_decode(base64_decode(base64_decode($_GET['branch_id'])));
        ?>
        <div class="page-wrapper">
            <div class="container-fluid">
                <div class="row heading-bg">
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                        <h5 class="txt-dark"><?=getField('name',$tbl_branch,$branch_id)?> Franchise Admin</h5>
                    </div>
                    ...
                    <div class="breadcrumb-section col-lg-4 col-sm-8 col-md-4 col-xs-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="active"><span><a href="#"><?=getField('name',$tbl_branch,$branch_id)?> Franchise Admin</a></span></li>
                        </ol>
                    </div>
                </div>
                <div class="row">
                                            ...
                                            <table id="datable_3" class="table table-hover display pb-30">
                                                ...
                                                <tbody>
                                                    ...
                                                    while($line=$obj->fetchNextObject($sql)){ 
                                                        $res_b_f = $obj->fetchNextObject($obj->query("select * from $tbl_branch_franchise where visa_sub_type='{$line->id}' and branch_id='$branch_id'"));
                                                        if($res_b_f->bv_per){
                                                        $bv_per = $res_b_f->bv_per;
                                                        $av_per = $res_b_f->av_per;
                                                    }else{
                                                            $bv_per = '0.00';
                                                            $av_per = '0.00';
                                                        }
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td> <input type="text" size="5" maxlength="5"
                                                                value="<?php echo $bv_per ?>"
                                                                onchange="chagnedisplayOrder(<?php echo $line->id; ?>,this.value, '<?=$line->amount?>', '<?=$line->after_visa_amount?>', '<?=$branch_id?>')">
                                                        </td>
                                                        <td> <input type="text" size="5" maxlength="5"
                                                                value="<?php echo $av_per ?>"
                                                                onchange="chagnedisplayOrders(<?php echo $line->id; ?>,this.value, '<?=$line->amount?>', '<?=$line->after_visa_amount?>', '<?=$branch_id?>')">
                                                        </td>
                                                        <td id="amount<?=$line->id?>">
                                                            <p>Without GST:
                                                                <?=$bv_per != 0 ? round(($line->amount * $bv_per)/100) : '0' ?>
                                                            </p>
                                                            <p> With GST:
                                                                <?=$bv_per != 0 ? round(($line->amount * $bv_per)/100 + ((($line->amount * $bv_per)/100)*18)/100) : '0' ?>
                                                            </p>
                                                        </td>
                                                        <td id="after_visa_amount<?=$line->id?>">
                                                            <p>Without GST:
                                                                <?=$av_per != 0 ? round(($line->after_visa_amount * $av_per)/100) : '0' ?>
                                                            </p>
                                                            <p> With GST:
                                                                <?=$av_per != 0 ? round(($line->after_visa_amount * $av_per)/100 + ((($line->after_visa_amount * $av_per)/100)*18)/100) : '0' ?>
                                                            </p>
                                                        </td>
                                                    </tr>
                                                    <?php
    ...
    function chagnedisplayOrder(id, ival, amount, after_visa_amount,branch_id) {
        $.ajax({
            type: "POST",
            url: 'controller.php',
            data: {
    ...
    function chagnedisplayOrders(id, ival, amount, after_visa_amount,branch_id) {
        $.ajax({
            type: "POST",
            url: 'controller.php',
            data: {
